...
class bnb(knapsack):
    ...
    def branch_and_bound(self, final_sol):
        ...
        temp = struc_sol()
        temp.fixed = 0
        self.frac_bound(temp, 0)
        self.insert(temp)
        self.QUIET = True
        while self.QueueSize != 0:
            head = self.removeMax()
            if head.fixed != n:
                head.fixed = head.fixed + 1
                h = head.copy()
                h.solution_vec[h.fixed] = 1
                self.frac_bound(h, h.fixed)
                infeasible = self.check_evaluate_and_print_sol(h.solution_vec)
                if infeasible is False:
                    self.insert(h)
                h = head.copy()
                h.solution_vec[h.fixed] = 0
                self.frac_bound(h, h.fixed)
                infeasible = self.check_evaluate_and_print_sol(h.solution_vec)
                if infeasible is False:
                    self.insert(h)
                ...
                self.QUIET = False
                self.copy_array(head.solution_vec, final_sol)
                return
